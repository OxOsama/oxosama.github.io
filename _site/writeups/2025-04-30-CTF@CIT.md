...
So, why don't try it on our server (https://ctf.cyber-cit.club/robots.txt), and **VoilÃ **, we get this file content:
```js
...
So, I submited it and **VoilÃ **, the flag is its first submitted sample name.
...
plt.scatter(bx, by, s=200, marker='s', color='blue', label='P (1â€“7)')
plt.scatter(rx, ry, s=200, marker='s', color='red', label='W (8â€“12)')
plt.scatter(gx, gy, s=200, marker='s', color='green', label='N (13â€“19)')
...
Inside the repo, there are only 7 commits, inside the commits page, we see a commit named `removed my API key`, so we open it, and **VoilÃ **, here is our flag:
