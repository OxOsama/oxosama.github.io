---
title: "OSINT Writeup â€” IEEE VICTORIS4 CTF Finals"
header:
  teaser: /assets/images/Covers/victoris4-finals.png
description: "After a full 6 hours in the `IEEE VICTORIS4 CTF` finals phase, Iâ€™m happy to share my write-up for three OSINT challenges I created: two Investigations (*Easy/Medium*) and one Geo (*Hard*), this is a step-by-step walkthrough aimed at beginners."
...
After a full 6 hours in the `IEEE VICTORIS4 CTF` finals phase, Iâ€™m happy to share my write-up for three OSINT challenges I created: two Investigations (*Easy/Medium*) and one Geo (*Hard*).
...
His masterpiece? rnicrosoft[.]com â€” a false mirror of trust, luring the careless.
One careless step led hunters to his trail.
>
>Follow the traces. Unmask the trickster. Gather his secrets.
>
>### Flag parts:
>â¦	First part (scammer name): the scammerâ€™s real full name.
>
>â¦	Second part (oldest domain): of the >16,390 domains registered under the scammerâ€™s name, identify the oldest domain registration (domain string only, e.g. example[.]com).
>
>â¦	Third part (leaks): how many data leaks (leaked datasets) were found associated with the scammerâ€™s accounts? (a numeric count)
>
...
>### Description
>
>Nothing beats a good sports time. Find where Iâ€™m and the next match will be on me.
...
First with some image-searching and comparing similar images, the construction style strongly suggests this place is in `Ø¹ÙŠÙ† Ø§Ù„Ø³Ø¨Ø¹ Ø¨Ø§Ù„Ù…ØºØ±Ø¨`.

![alt text](/assets/images/Write-Ups/victoris4_OSINT_finals/AÃ¯n-SebaÃ¢.JPG)
...
After some edit to the image and giving it to any AI agent, we can conclude that the phrase is `Ø§Ù„ÙØ¶Ø§Ø¡ Ø§Ù„Ø±ÙŠØ§Ø¶ÙŠ Ø¥Ù‚Ø§Ù…Ø© Ø§Ù„Ø´Ø¨Ø§Ø¨`, so we search on it on the maps and start search for any playgrounds in the found area till we find this:
